...
if current_headers != EXPECTED_HEADERS:
    raise RuntimeError(
        ...
        "Сначала запустите update_table_v6.py, затем снова запустите bot.py."
    )
...
def take_lead_in_sheet(lead_id: str, manager: str) -> tuple[bool, str]:
    ...
    status = (leads_sheet.cell(cell.row, 8).value or "").strip()
    current_manager = (leads_sheet.cell(cell.row, 9).value or "").strip()
    ...
    leads_sheet.update_cell(cell.row, 8, "В работе")
    leads_sheet.update_cell(cell.row, 9, manager)
    leads_sheet.update_cell(cell.row, 10, taken_at)
    return True, manager
...
async def submit(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    row = [
        ...
        data["car"],                   # E Автомобиль
        data["installment"],           # F Рассрочка
        data.get("source", "direct"),  # G Источник
        "Новый",                       # H Статус
        "",                            # I Менеджер
        "",                            # J Взято в работу
        "",                            # K Комментарий
        username,                      # L Telegram
        callback.from_user.id,         # M Telegram ID
    ]
