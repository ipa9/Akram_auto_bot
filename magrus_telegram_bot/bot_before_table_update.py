...
# В колонке I раньше хранился бюджет. Теперь здесь фиксируется, нужна ли клиенту рассрочка.
if (leads_sheet.cell(1, 9).value or "").strip() != "Рассрочка":
    leads_sheet.update_cell(1, 9, "Рассрочка")

# Эти колонки больше не являются ответами клиента: в них фиксируются базовые условия рассрочки.
for col, title in [(10, "Первый взнос (условия)"), (11, "Срок (условия)"), (12, "Поручители (условия)")]:
    if (leads_sheet.cell(1, col).value or "").strip() != title:
        leads_sheet.update_cell(1, col, title)
...
async def submit(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    row = [
        ...
        data["installment"],           # I Рассрочка
        data["payment"],               # J Условия первого взноса
        data["term"],                  # K Условия срока
        data["guarantors"],            # L Условия по поручителям
